...
<div class="kt-content  kt-grid__item kt-grid__item--fluid p-0" id="kt_content">
    ...
    <div class="kt-portlet kt-portlet--mobile">
        @if (Helper::permission('company-module-bulk-action-delete') || Helper::permission('company-module-report'))
        <div class="row py-3">
            <div class="dataTableButton text-right col-md-12 col-sm-12 px-0 d-flex justify-content-center">
                ...
                @endif
                @if (Helper::permission('company-module-bulk-action-delete'))
        ...
        <div class="kt-portlet__head kt-portlet__head--lg">
            ...
            <div class="kt-portlet__head-toolbar">
                <div class="kt-portlet__head-wrapper">
                    ...
                    <button type="button" id="showModal" onclick="show_modal(modal_title='Add New Module',btn_text='Save')" class="btn btn-brand btn-icon-sm btn-sm">
            ...
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    ...
                    <form method="POST" id="saveDataForm">
                        @csrf
                        <input type="hidden" class="form-control" name="module_id" id="update_id">
                        <div class="modal-body">
                            <div class="form-group row  required">
                                <div class="col-lg-6">
                                    <label for="module_name" class="form-control-label">Module Name</label>
                                    <input type="text" class="form-control" name="module_name" id="module_name" placeholder="Enter module name" onkeyup="url_generator(this.value, output_id='module_link')">
                                </div>
                                <div class="col-lg-6">
                                    <label for="module_link" class="form-control-label">Module Link</label>
                                    <input type="text" class="form-control" name="module_link" id="module_link">
                                </div>
                            </div>
                            <div class="form-group row  required">
                                <div class="col-lg-6">
                                    <label for="module_icon" class="form-control-label">Module Icon</label>
                                    <input type="text" class="form-control" name="module_icon" id="module_icon" placeholder="fas fa-plus">
                                </div>
                                <div class="col-lg-6">
                                    <label for="module_sequence" class="form-control-label">Module Sequence</label>
                                    <input type="text" class="form-control" name="module_sequence" id="module_sequence"  placeholder="Enter module sequence">
                                </div>
                            </div>
...
$(document).ready(function () {
    ...
    var table = $('#dataTable').DataTable({
        ...
        "ajax": {
            "url": "{{route('admin.company.module.list')}}",
            "type": "POST",
            "data": function (data) {
    ...
    $(document).on('click','#save-btn',function () {
        ...
        if(id){
            var method = 'update';
            var url = "{{route('admin.company.module.update')}}";
        }else{
            var method = 'add';
            var url = "{{route('admin.company.module.store')}}";
        }
        $.ajax({
            url: url,
            type: "POST",
    ...
    $(document).on('click','.edit_data',function () {
        ...
        $('.selectpicker').selectpicker('refresh');
        $.ajax({
            url: "{{route('admin.company.module.edit')}}",
            type: "POST",
            ...
            success: function (data) {
                ...
                $('.modal-title').html('<i class="fas fa-edit"></i> <span>Edit Module</span>');
    ...
    $(document).on("change",".change_status", function() {
        ...
        if(status && id){
            $.ajax({
                url: "{{route('admin.company.module.change-status')}}",
                type: "POST",
    ...
    $(document).on('click','.delete_data',function () {
        ...
        var id  = $(this).data('id');
        var url = "{{route('admin.company.module.delete')}}";
        Swal.fire({
            title: 'Are you sure?',
    ...
    $(document).on("click",'#bulk_action_delete',function(e) {
        ...
        if(id.length === 0) //tell us if the array is empty
        {
            ...
            var url = "{{route('admin.company.module.bulkaction')}}";
            Swal.fire({
                title: 'Are you sure?',
                text: "You won't be able to revert this!",
                type: 'warning',
