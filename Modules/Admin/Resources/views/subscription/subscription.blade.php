...
<div class="kt-content  kt-grid__item kt-grid__item--fluid p-0" id="kt_content">
    ...
    <div class="kt-portlet kt-portlet--mobile">
        @if (Helper::permission('subscription-bulk-action-delete') || Helper::permission('subscription-report'))
        <div class="row py-3">
            <div class="dataTableButton text-right col-md-12 col-sm-12 px-0 d-flex justify-content-center">
               
                @if (Helper::permission('subscription-report'))
        ...
        <div class="kt-portlet__head kt-portlet__head--lg">
            ...
            <div class="kt-portlet__head-toolbar">
                <div class="kt-portlet__head-wrapper">
                    ...
                    @if (Helper::permission('subscription-add'))
                    <button type="button" id="showModal" onclick="show_modal(modal_title='Add New Subscription',btn_text='Save')" class="btn btn-brand btn-icon-sm btn-sm">
                    ...
                    <form method="POST" id="saveDataForm">
                        ...
                        <div class="modal-body">
                            <div class="form-group row  required">
                                ...
                                <div class="col-lg-6 mb-2">
                                    <label for="duration_month" class="form-control-label">Subscription Duration (Total Month)</label>
...
$(document).ready(function () {
    ...
    $(document).on('click','#save-btn',function () {
        ...
        if(id){
            var method = 'update';
            var url = "{{route('admin.subscription.update')}}";
        }else{
            var method = 'add';
            var url = "{{route('admin.subscription.store')}}";
        }
    ...
    $(document).on('click','.edit_data',function () {
        ...
        $.ajax({
            url: "{{route('admin.subscription.edit')}}",
            type: "POST",
            data:{id:id,_token:_token},
            dataType: "JSON",
            success: function (data) {
                console.log(data.method);
                $('#saveDataForm #update_id').val(data.subscription.id);
                $('#saveDataForm #type').val(data.subscription.type);
                $('#saveDataForm #total_branch_account').val(data.subscription.total_branch_account);
                $('#saveDataForm #total_user_account').val(data.subscription.total_user_account);
                $('#saveDataForm #amount').val(data.subscription.amount);
                $('#saveDataForm #duration_month').val(data.subscription.duration_month);
                $('#saveDataModal').modal({
                    keyboard: false,
    ...
    $(document).on('click','.delete_data',function () {
        ...
        var id  = $(this).data('id');
        var url = "{{route('admin.subscription.delete')}}";
        Swal.fire({
            title: 'Are you sure?',
    ...
    $(document).on("click",'#bulk_action_delete',function(e) {
        ...
        if(id.length === 0) //tell us if the array is empty
        {
            ...
            var url = "{{route('admin.subscription.bulkaction')}}";
            Swal.fire({
                title: 'Are you sure?',
                text: "You won't be able to revert this!",
                type: 'warning',
