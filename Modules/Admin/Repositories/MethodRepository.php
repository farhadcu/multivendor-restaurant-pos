...
use Modules\Admin\Entities\AdminMethodPermission;
...
class MethodRepository extends BaseRepository implements MethodContract
{
    ...
    {
        $this->rules['method_slug']  = 'required|string|unique:methods';
    ...
    public function updateMethod(array $params)
    {
            if(!empty($params['method_id'])){
                ...
                $this->rules['method_slug']  = 'required|string|unique:methods,method_slug,'.$method->id;
                $validator  = Validator::make($params, $this->rules);
                if ($validator->fails()) {
                    $output = array(
                        'errors' => $validator->errors()
    ...
    public function deleteMethod(array $params)
    {
        if(!empty($params['id'])){
            $this->id     = Crypt::decrypt($params['id']);
            AdminRoleMethodPermission::where('method_id',$this->id)->delete();
            AdminMethodPermission::where('method_id',$this->id)->delete();
    ...
    public function bulk_action_delete(array $params)
    {
        ...
        if(!empty($this->id) && count($this->id) > 0){
            AdminRoleMethodPermission::whereIn('method_id',$this->id)->delete();
            AdminMethodPermission::whereIn('method_id',$this->id)->delete();
