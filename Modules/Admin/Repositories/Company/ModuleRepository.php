...
use Modules\Admin\Contracts\Company\ModuleContract;
...
class ModuleRepository extends BaseRepository implements ModuleContract
{
    ...
    public function createModule(array $params)
    {
        ...
        if($params['module_link'] == 'javascript:void(0);'){
            ...
            $this->rules['module_link']  = 'required|string|unique:company_modules';
        }
    ...
    public function updateModule(array $params)
    {
        if(!empty($params['module_id'])){
            ...
            if($params['module_link'] == 'javascript:void(0);'){
                ...
                $this->rules['module_link']   = 'required|string|unique:company_modules,module_link,'.$module->id;
            }
            $validator                    = Validator::make($params, $this->rules);
            if ($validator->fails()) {
    ...
    public function deleteModule(array $params)
    {
        if(!empty($params['id'])){
            $this->id = Crypt::decrypt($params['id']);
            CompanyMethod::where('module_id',$this->id)->delete();
            CompanyRoleModulePermission::where('module_id',$this->id)->delete();
            UserModulePermission::where('module_id',$this->id)->delete();
    ...
    public function bulk_action_delete(array $params)
    {
        ...
        if(!empty($this->id) && count($this->id) > 0){
            CompanyMethod::whereIn('module_id',$this->id)->delete();
            CompanyRoleModulePermission::whereIn('module_id',$this->id)->delete();
            UserModulePermission::whereIn('module_id',$this->id)->delete();
