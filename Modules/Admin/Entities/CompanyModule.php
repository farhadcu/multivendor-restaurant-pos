...
{
    use NestableTrait;
    protected $fillable = ['module_name','module_link','module_icon','module_sequence','parent_id','status'];
     ...
     **********************************************/
    protected $_table_name     = 'company_modules'; //set table name
     ...
     **********************************************/
     ...
     public function parent_name($parent_id){
        $parent = self::select('module_name')->where('id',$parent_id)->first();
        if(!empty($parent)){
