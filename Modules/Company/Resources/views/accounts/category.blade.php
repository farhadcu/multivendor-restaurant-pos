...
<div class="kt-content  kt-grid__item kt-grid__item--fluid p-0" id="kt_content">
    ...
    <div class="kt-portlet kt-portlet--mobile">
        @if (Helper::permission('income-category-bulk-action-delete') || Helper::permission('income-category-report')
        || Helper::permission('expense-category-bulk-action-delete') || Helper::permission('expense-category-report'))
        <div class="row py-3">
            <div class="dataTableButton text-right col-md-12 col-sm-12 px-0 d-flex justify-content-center">
               
                @if (Helper::permission('income-category-report') || Helper::permission('expense-category-report'))
        ...
        <div class="kt-portlet__head kt-portlet__head--lg">
            ...
            <div class="kt-portlet__head-toolbar">
                <div class="kt-portlet__head-wrapper">
                    @if (Helper::permission('income-category-add') || Helper::permission('expense-category-add'))
                        ...
                        <form method="POST" id="form-filter" class="m-form m-form--fit m--margin-bottom-20" role="form">
                            <div class="row align-items-center">
                                <div class="form-group col-md-3 kt-margin-b-20-tablet-and-mobile">
                                    ...
                                    <input type="text" class="form-control" name="category_name" id="category_name" placeholder="Enter supplier name" />
                                </div>
                                <div class="form-group col-md-3 kt-margin-b-20-tablet-and-mobile">
                                    <label>Status</label>
        ...
        <div class="kt-portlet__body pt-0">
            ...
            <table class="table table-striped- table-bordered table-hover table-checkable" id="dataTable">
                <thead>
                    <tr>
                        @if (Helper::permission('income-category-bulk-action-delete'))
            ...
            <div class="modal-dialog modal-md" role="document">
                <div class="modal-content">
                    ...
                    <form method="POST" id="saveDataForm">
                        ...
                        <input type="hidden" class="form-control" name="type" id="type" value="{{$data['type']}}">
                        <div class="modal-body">
                            <div class="form-group required">
                                <label for="category_name" class="form-control-label">Name</label>
                                <input type="text" class="form-control" name="category_name" id="category_name" placeholder="Enter account title">
                            </div>
...
$(document).ready(function () {
    ...
    var table = $('#dataTable').DataTable({
        ...
        "ajax": {
            "url": "{{route('account.category.list')}}",
            "type": "POST",
            "data": function (data) {
    ...
    buttons: [     
            ...
            },
            {
                extend: 'excel',
                title: "{{ucwords($sub_title)}}",
                filename: 'income-category-report',
                exportOptions: {
                     @if (Helper::permission('income-category-bulk-action-delete') || Helper::permission('expense-category-bulk-action-delete'))
                    columns: [1,2,3,4]
                    @else
            ...
            },
            {
                ...
                customize: function ( doc ) {
                    doc.content[1].table.widths = ['5%','30%','55%','10%'];
    ...
    $(document).on('click','#save-btn',function () {
        ...
        if(id){
            var method = 'update';
            var url = "{{route('account.category.update')}}";
        }else{
            var method = 'add';
            var url = "{{route('account.category.store')}}";
        }
    ...
    $(document).on('click','.edit_data',function () {
        ...
        $.ajax({
            url: "{{route('account.category.edit')}}",
            type: "POST",
            ...
            success: function (data) {
                ...
                $('#saveDataForm #description').val(data.category.description);
                $('#saveDataForm #status').val(data.category.status);
    ...
    $(document).on('click','.view_data',function () {
        ...
        $.ajax({
            url: "{{route('account.category.view')}}",
            type: "POST",
            data:{id:id,_token:_token},
            dataType: "JSON",
            success: function (data) {
                ...
                $('.modal-title').html('<i class="kt-nav__link-icon flaticon2-expand"></i> <span> View Category Data</span>');
    ...
    $(document).on('click','.delete_data',function () {
        ...
        var id  = $(this).data('id');
        var url = "{{route('account.category.delete')}}";
    ...
    $(document).on("click",'#bulk_action_delete',function(e) {
        ...
        if(id.length === 0) //tell us if the array is empty
        {
            ...
            var url = "{{route('account.category.bulkaction')}}";
