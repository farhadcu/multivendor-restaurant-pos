...
<div class="kt-content  kt-grid__item kt-grid__item--fluid p-0" id="kt_content">
    ...
    <div class="kt-portlet kt-portlet--mobile">
        <form method="POST" id="saveDataForm">
            @csrf
            <input type="hidden" name="update_id" value="{{$data['purchase']['id']}}">
                ...
                <p class="italic text-danger">The field labels marked with (*) are required input fields.</p>
                                ...
                                <select class="form-control selectpicker" name="supplier_id" id="supplier_id" data-live-search="true" data-live-search-placeholder="Search" title="Select Please">
                                    ...
                                    @if (!empty($data['suppliers']))
                                        @foreach ($data['suppliers'] as  $value)
                                            <option @if($data['purchase']['supplier_id'] == $value->id) {{'selected'}} @endif value="{{$value->id}}">{{$value->supplier_name.' ('.$value->supplier_company_name.')'}}</option>
                                         @endforeach
                                    @endif
                            ...
                            <div class="form-group col-md-4 required">
                                ...
                                <select class="form-control selectpicker" name="status" id="status" onchange="show_received_qty(this.value)" data-live-search="true" data-live-search-placeholder="Search" title="Select Please">
                                    ...
                                    @foreach (PURCHASE_STATUS as $key => $value)
                                        <option @if($data['purchase']['status'] == $key) {{'selected'}} @endif value="{{$key}}">{{$value}}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered" style="font-size:12px;" id="cart_table">
                                <thead>
                                    ...
                                    <th width="10%" class="text-center font-weight-bold received_qty @if($data['purchase']['status'] == 1) {{'d-none'}} @endif">Received</th>
                        ...
                        <div class="row mt-5">
                            <div class="form-group col-md-3">
                                ...
                                <input type="text" class="form-control" oninput="calculate_grand_total()" name="order_discount" id="order_discount" value="{{$data['purchase']['order_discount']}}" style="text-align:right">
                            </div>
                            <div class="form-group col-md-3">
                                <label for="">Tax</label>
                                <input type="text" class="form-control" oninput="calculate_grand_total()" name="order_tax_amount" id="order_tax_amount" value="{{$data['purchase']['order_tax_amount']}}" style="text-align:right">
                            </div>
                            <div class="form-group col-md-3">
                                <label for="">Shipping Cost</label>
                                <input type="text" class="form-control" oninput="calculate_grand_total()" name="shipping_cost" id="shipping_cost" value="{{$data['purchase']['shipping_cost']}}" style="text-align:right">
                            </div>
                            <div class="form-group col-md-3">
                                <label for="">Grand Total</label>
                                <input type="text" class="form-control" name="grand_total" id="grand_total" value="{{$data['purchase']['grand_total']}}" style="text-align:right;" readonly>
                            </div>

                            <div class="form-group col-md-12">
                                <label for="note">Note</label>
                                <textarea rows="5" class="form-control" name="note" id="note">{{$data['purchase']['note']}}</textarea>
                            </div>
                        </div>
