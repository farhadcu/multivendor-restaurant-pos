...
class UserRepository extends BaseRepository implements UserContract
{
    ...
    private $id;
    private $type = 2; //type = 2 is for company admin
    ...
    public function getList(array $params)
    {
        ...
        $this->model->setCompany(auth()->user()->company_id);
        $this->model->setBranch(session()->get('branch'));
        ...
        foreach ($list as $value) {
            ...
            if($this->helper->permission('user-edit')){
                $action .= '<li class="kt-nav__item"><a class="kt-nav__link" href="' . url('user/edit',Crypt::encrypt($value->id)) . '" >'.EDIT_ICON.'</a></li>';
            }
            ...
            if($this->helper->permission('user-delete')){
            ...
            $row    = array();
            if($this->helper->permission('user-bulk-action-delete')){
    ...
    public function createUser(array $params)
    {
        ...
        if ($validator->fails()) {
            ...
            $total_user = $this->model->where(['company_id' => auth()->user()->company_id])->count();
            ...
            if($total_user >= $company_subscription->total_user_account){
                ...
                if($params->has('module') && $params->has('method')){
                    ...
                    $history[]  = [
                        ...
                        'text'  => "Data created by ".auth()->user()->name,
                        'date'  => date('d-M-Y',strtotime(DATE)),
                        'mobile'=> auth()->user()->mobile
                    ];
                    $history    = json_encode($history);
                    $merge      = $collection->merge(compact('company_id','branch_id','password','history'));
    ...
    public function updateUser(array $params)
    {
        if(!empty($params['user_id'])){
            ...
            $this->rules['email']                 = 'required|string|email|max:100|unique:users,email,'.$this->id;
            $this->rules['mobile']                = ['required', new ValidPhone, 'unique:users,mobile,'.$this->id];
            $validator                            = Validator::make($params, $this->rules);
            if ($validator->fails()) {
                $output                           = array(
                    'errors' => $validator->errors()
                ...
                if($params->has('module') && $params->has('method')){
                    ...
                    $history[]   = [
                        ...
                        'text'  => "Data updated by ".auth()->user()->name,
                        'date'  => date('d-M-Y',strtotime(DATE)),
                        'mobile'=> auth()->user()->mobile
                    ];
                    $history                      = json_encode($history);
                    ...
                    if ($result) {
                        ...
                        UserModulePermission::where(['company_id'=>$company_id,'user_id'=>$this->id])->delete();
                        UserMethodPermission::where(['company_id'=>$company_id,'user_id'=>$this->id])->delete();
                        $user_module_permission   = [];
                        $user_method_permission   = [];
                        foreach ($params['module'] as $module) {
                            $user_module_permission[] = [
        ...
        if(!empty($params['id'])){
            ...
            if((int)$params['status']){
                ...
                $history[]    = [
                    ...
                    'text'  => "Status changed by ".auth()->user()->name,
                    'date'  => date('d-M-Y',strtotime(DATE)),
                    'mobile'=> auth()->user()->mobile
                ];
    ...
    private function multilevel_permission($parent_id = NULL,$user_id=NULL)
    {
        ...
        if(!empty($modules)){
            foreach ($modules as $value) {
                $user_permitted_module = UserModulePermission::where(['company_id' => auth()->user()->company_id,'module_id'=>$value->id,'user_id'=>$user_id])->first(); //check module existance in role_module_permissins table
                ...
                foreach ($methodlist as $method) {
                    $user_permitted_method  = UserMethodPermission::where(['company_id' => auth()->user()->company_id,'method_id'=>$method->id,'user_id'=>$user_id])->first(); //check method existance in role_method_permissions table
