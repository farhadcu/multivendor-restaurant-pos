...
use Validator;
use Modules\Company\Entities\Accounts\TransactionCategory;
use Illuminate\Support\Facades\Crypt;
use Modules\Company\Contracts\Accounts\TransactionCategoryContract;

class TransactionCategoryRepository extends BaseRepository implements TransactionCategoryContract
{
    private $rules = [
    ...
    public function index($type)
    {
        ...
        'branch_id'=>session()->get('branch'),'type'=>$type])
                            ->orderBy('id','asc')->get();
    }
    ...
    {      
        ...
        if ($validator->fails()) {
            ...
            $history[] = [
                ...
                'text'   => 'Category created by '.auth()->user()->name,
                'date'   => DATE_FORMAT,
        ...
        if(!empty($params['update_id'])){
            ...
            if ($validator->fails()) {
                ...
                $history[] = [
                    ...
                    'text'   => 'Category updated by '.auth()->user()->name,
                    'date'   => DATE_FORMAT,
                    'mobile' => auth()->user()->mobile
                ];
                $history      = json_encode($history);
