...
class RoleRepository extends BaseRepository implements RoleContract
{
    ...
    public function createRole(array $params)
    {
        ...
        if ($validator->fails()) 
        {
            ...
            $role_name_exist    = $this->model->where(['company_id'=>auth()->user()->company_id,'role'=>$params['role']])->first();
            if(!empty($role_name_exist)){
                $output         = array( 'errors' => ['role'=>'This role name is already taken. Try another'] );
            }else{
                $collection     = collect($params);
                $company_id     = auth()->user()->company_id;
                $merge          = $collection->merge(compact('company_id'));
    ...
    public function updateRole(array $params)
    {
        if(!empty($params['role_id'])){
            ...
            if ($validator->fails()) 
            {
                ...
                $role_name_exist        = $this->model->where(['company_id'=>auth()->user()->company_id,'role'=>$params['role']])->count();
